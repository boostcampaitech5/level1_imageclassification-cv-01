...
from dataset_with_path import MaskBaseDataset
...
def train(data_dir, model_dir, args):
    ...
    y_train=[int(labels) for _, labels, _ in train_set]
    weight = 1. / np.array([2745, 2050, 415, 3660, 4085, 545, 549, 410, 83, 732, 817, 109, 549, 410, 83, 732, 817, 109] )
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                answer.extend(labels.cpu().numpy())
            ...
            if val_acc > best_val_acc:
                ...
                if args.infer:
                    ...
                    df['preds'] = prediction
                    df["answer"] = answer
                    df["preds_mask"] = df["preds"] // 6
                    df["answer_mask"] = df["answer"] // 6
                    df["preds_gender"] = (df["preds"] % 6) // 3
                    df["answer_gender"] = (df["answer"] % 6) // 3
                    df["preds_age"] = df["preds"] % 3
                    df["answer_age"] = df["answer"] % 3
                    df.to_csv(f"{save_dir}/pred.csv")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--CEloss', type=float, default=0, help='weight of cross_entropy')
    parser.add_argument('--FOloss', type=float, default=0.4, help='weight of FocalLoss')
    parser.add_argument('--LAloss', type=float, default=0.4, help='weight of LabelSmoothingLoss')
    parser.add_argument('--F1loss', type=float, default=0.2, help='weight of F1Loss')
